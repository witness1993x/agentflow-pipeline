...
5. Lark cards can deep-link to TG via `--lark-cta-tg-bot @bot_username` or `LARK_CTA_TG_BOT`; the promoted button becomes `https://t.me/<bot>?start=case_HSP-XXX_dry_publish`, and `agentflow-tg-listen` now handles that `/start` payload.
